...
3. The DBViewer page had all its CSS and JavaScript embedded in `dbviewer.html` itself, which made the page unhygienic to review and impossible to serve with a strict CSP.
...
- **Extract `dbviewer.html` inline CSS to `dbviewer.css` and inline JS to `dbviewer.js`.** Both files are referenced from `dbviewer.html` via `<link>` and `<script src>`. This makes the viewer's behavior reviewable in isolation, cacheable, and CSP-friendly.
- **Update `tests/dbviewer.test.js`** to assert the new structure: it now reads `dbviewer.js` and `dbviewer.css` for the script/CSS contracts, and asserts that `dbviewer.html` contains neither `<script>` nor `<style>` blocks.
...
- **Update `AGENTS.md`** to document the new `dbviewer.js` and `dbviewer.css` files in the project-structure list.
...
- `dbviewer`: the page's CSS and JS are no longer embedded; `dbviewer.html` must reference `dbviewer.css` and `dbviewer.js`, and the DOM/test contract for the viewer now spans three files instead of one. The `Required DOM scaffolding` and `Internal helpers` requirements are updated to reflect this.
...
- `dbmapper.html`: new CSP meta tag, SRI on the Materialize CSS and JS tags, the welcome-screen CTA gains `id="btn-import-sql-cta"` and loses its `onclick` attribute. One new file (`dbviewer.js`) is referenced from `dbviewer.html`. No behavioral change.
- `dbviewer.html`: trimmed from ~1460 lines to ~115 lines; the inline `<style>` and `<script>` blocks are replaced by `<link rel="stylesheet" href="dbviewer.css">` and `<script src="dbviewer.js"></script>`. A new CSP meta tag is added in the head.
- `dbviewer.js`, `dbviewer.css`: new files at the repo root, holding the extracted viewer code.
- `app.js`: one new event listener (5 lines) for the welcome-screen CTA button.
- `tests/dbviewer.test.js`: rewritten to assert the new file structure (HTML has no inline script/style, functions live in `dbviewer.js`, the `body.theme-dark` class lives in `dbviewer.css`). Same exit code, same `ok` line.
- `SECURITY.md`: the three "Limitações conhecidas" items for SRI, CSP, and inline DBViewer JS/CSS are removed and replaced with three honest new limitations. Five new "what the app does" items are added under the "O que o app faz e não faz" section.
- `AGENTS.md`: the project-structure bullet list is updated to include `dbviewer.html`, `dbviewer.css`, and `dbviewer.js`.
- `openspec/specs/`: a new `static-site-security/spec.md` is created. `dbviewer/spec.md` is updated (the `Internal helpers` requirement is rewritten to point at `dbviewer.js`). The other six specs each gain one new "Cross-cutting: runs under static-site-security" requirement.
