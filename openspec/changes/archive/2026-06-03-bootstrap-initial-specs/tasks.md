...
- [x] 2.2 Read `specs/project-persistence/spec.md` and confirm the data model, export, import, and backfill requirements match `app.js` and the JSON shape consumed by `dbviewr.html`
...
- [x] 2.7 Read `specs/dbviewr/spec.md` and confirm every requirement matches the DOM contracts and the `REMOVED`/search/relationships/theme rules exercised by `tests/dbviewr.test.js`
...
- [x] 3.2 Run `node tests/dbviewr.test.js` and confirm it exits 0 — this validates `dbviewr`
...
- [ ] 5.2 Run `openspec list --specs` and confirm seven capabilities are listed: `sql-import`, `project-persistence`, `schema-tree`, `semantic-editor`, `coverage-stats`, `model-update-wizard`, `dbviewr`
