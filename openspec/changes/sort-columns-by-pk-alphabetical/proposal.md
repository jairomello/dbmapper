...
- The reordering applies to the viewer surface only (`dbviewer.html` / `dbviewer.js`). The editor (`dbmapper.html`) preserves the original SQL order, since that reflects the schema definition.
...
- `dbviewer`: The column rendering order changes from SQL-definition order to PK-first + alphabetical. This is a spec-level behavior change because the rendered output order is part of the viewer's contract.

## Impact

- **Code**: `dbviewer.js` — the function that renders a table's column list needs to sort columns before rendering.
- **Tests**: `tests/dbviewer.test.js` — may need updates if it asserts column order.
